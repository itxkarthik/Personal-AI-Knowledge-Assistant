...
export function SearchResults({ query, results, total, isLoading = false }: SearchResultsProps) {
  ...
  if (!query.trim()) {
    return <section className="border border-border bg-muted p-5 text-sm text-muted-foreground">Run a search to scan documents, notes, and chats in one place.</section>;
  }

  if (results.length === 0) {
    return <section className="border border-border bg-muted p-5 text-sm text-muted-foreground">No results found for {query}.</section>;
  }

  return (
    <section className="space-y-3">
      <p className="text-xs text-muted-foreground">{total} {total === 1 ? "result" : "results"}</p>
          ...
          <Link key={`${result.entity_type}-${result.id}-${result.updated_at ?? ""}`} href={href} className="block border border-border bg-background p-4 hover:bg-muted">
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                ...
                <h3 className="mt-1 line-clamp-1 text-sm font-bold text-foreground">{result.title || "Untitled"}</h3>
                <p className="mt-1 line-clamp-2 text-xs text-muted-foreground">{result.snippet || "No preview available"}</p>
              </div>
