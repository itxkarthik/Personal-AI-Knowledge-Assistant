...
def is_access_token_blacklisted(session: Session, jti: str) -> bool:
    ...
    """
    return crud.is_token_blacklisted(session=session, jti=jti)
...
def cleanup_expired_tokens(session: Session) -> int:
    ...
    refresh_deleted = refresh_result.rowcount or 0
    ...
    blacklist_deleted = blacklist_result.rowcount or 0
...
def get_token_info(session: Session, token_payload: dict) -> dict:
    """
