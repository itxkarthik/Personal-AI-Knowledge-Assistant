...
def run_rag_pipeline(*, session: Session, user_id: int, query: str) -> RAGResult:
	if not query.strip():
		return RAGResult(answer="Please provide a non-empty question.", sources={"documents": [], "chunks": []})
	...
	document_map = _load_document_map(session=session, document_ids=[item.document_id for item in chunk_hits])
...
def _build_context_chunks(*, chunk_hits: list[VectorSearchResult], document_map: dict[int, Document]) -> list[str]:
...
def _build_sources_payload(*, chunk_hits: list[VectorSearchResult], document_map: dict[int, Document]) -> dict[str, Any]:
	...
	documents = sorted(document_summary.values(), key=lambda item: float(item["max_score"]), reverse=True)
